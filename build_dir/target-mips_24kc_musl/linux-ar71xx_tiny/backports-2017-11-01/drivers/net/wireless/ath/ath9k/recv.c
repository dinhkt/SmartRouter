...
u8 check_MAC[6];
bool check_client_signal_strength = false;
int packet_signal[10];
int packet_signal_count = 0;

DEFINE_SPINLOCK(client_info_list_lock);
struct packet_mac_signal client_signal_info;
bool is_client_signal_info_initialized = false;
...
// ###############################################################
void print_MAC(u8 *addr) {
	printk("\n### recv.c: %2x:%2x:%2x:%2x:%2x:%2x", addr[0], addr[1], addr[2], addr[3], addr[4], addr[5]);
}

void printAtenna(struct ieee80211_rx_status *status) {
	printk("\n@@@ recv.c Signal: %d", status->signal);
}
...
void prom_start(u8 *addr) {
	int i;
	for(i = 0; i < 6; i++) {
		check_MAC[i] = addr[i];
	}
	check_client_signal_strength = true;
}
EXPORT_SYMBOL(prom_start);
int packet_scan() {
	printk("\n\n packet scan DEBUGGGGGGGGGG");
	int i;
	int sum = 0;
	if (packet_signal_count == 0) {
		for (i = 0; i < 10; ++i) {
			sum += packet_signal[i];
		}
		return sum/10;
...
void prom_stop() {
	check_client_signal_strength = false;
	packet_signal_count = 0;
}

int take_packet_value(u8 *addr) {
	
}
EXPORT_SYMBOL(packet_scan);

struct packet_mac_signal get_client_signal_info(void){
	return client_signal_info;
}
EXPORT_SYMBOL(get_client_signal_info);

void reset_client_signal_info(void) {
	printk("\n RESET");
	struct list_head *node, *temp;
	struct packet_mac_signal *current_node;
	int i = 0;
	list_for_each_safe(node, temp, &(client_signal_info.list)){
		printk("\n enter loop");
...
EXPORT_SYMBOL(reset_client_signal_info);

inline void mac_copy(u8 * dst, const u8* src) {
	int i = 0;
	for (i = 0; i < 6; i++) {
		dst[i] = src[i];
	}
...
void process_client_signal_info(u8* MACaddr, int signal) {
	...
	list_for_each_safe(node, temp, &(client_signal_info.list)){
		current_node = list_entry(node,struct packet_mac_signal,list);
		if (MAC_compare(current_node->MACaddr, MACaddr)) {
			is_existed = true;
			if (signal < GOOD_SIGNAL_THRESHOLD) {
				current_node->signal = signal;
			} else {
				list_del(node);
				kfree(current_node);
			}
	...
	if(!is_existed && signal < GOOD_SIGNAL_THRESHOLD) {
		struct packet_mac_signal *client_signal_info_entry = kmalloc(sizeof(struct packet_mac_signal *), GFP_KERNEL);
		mac_copy(client_signal_info_entry->MACaddr, MACaddr);
		client_signal_info_entry->signal = signal;
		list_add_tail(&client_signal_info_entry->list, &(client_signal_info.list));	
	}
}

void init_list_client_signal_info(void) {
	INIT_LIST_HEAD(&(client_signal_info.list));
}

void lock_client_info_list(void) {
	spin_lock(&client_info_list_lock);
}
EXPORT_SYMBOL(lock_client_info_list);

void unlock_client_info_list(void) {
	spin_unlock(&client_info_list_lock);
}
EXPORT_SYMBOL(unlock_client_info_list);
...
int ath_rx_tasklet(struct ath_softc *sc, int flush, bool hp)
{
	...
	do {
		...
		if (!is_client_signal_info_initialized) {
			init_list_client_signal_info();
			is_client_signal_info_initialized = true;
		}

		if (spin_trylock(&client_info_list_lock)) {
			process_client_signal_info(hdr->addr2, rxs->signal);
			spin_unlock(&client_info_list_lock);
		}

		if (check_client_signal_strength) {
			ath9k_hw_setrxfilter(sc->sc_ah, ATH9K_RX_FILTER_UCAST | ATH9K_RX_FILTER_BCAST
				| ATH9K_RX_FILTER_MCAST | ATH9K_RX_FILTER_PROM);
		}

		if (check_client_signal_strength && MAC_compare(check_MAC, hdr->addr2)) {
			if( packet_signal_count < 10) {
				packet_signal[packet_signal_count] = rxs->signal;
				packet_signal_count++;
			} else if(packet_signal_count == 10) {
				prom_stop();
				ath9k_hw_setrxfilter(sc->sc_ah, ATH9K_RX_FILTER_UCAST | ATH9K_RX_FILTER_BCAST
					| ATH9K_RX_FILTER_MCAST);
			}
