...
/*
	CHECK PACKET ATENNA STRENGTH
*/
//###############################################################
#define GOOD_SIGNAL_THRESHOLD -0

void prom_start(u8* addr);
int packet_scan(void);
void prom_stop(void);

// void set_disconnect_client_MAC(u8* addr);
void start_disconnect_client_MAC(u8 * addr);
void finish_disconnect_client_MAC(u8 * addr);
...
void mac_copy(u8 * dst, const u8* src);
//###########################################################33
