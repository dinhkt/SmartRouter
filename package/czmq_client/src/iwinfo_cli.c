...
char * format_bssid(unsigned char *mac)
{
	...
	snprintf(buf, sizeof(buf), "0x%02X:0x%02X:0x%02X:0x%02X:0x%02X:0x%02X",
...
void print_scanlist(const struct iwinfo_ops *iw, const char *ifname)
{
	int i, x, len;
...
void print_assoclist(const struct iwinfo_ops *iw, const char *ifname)
{
	int i, len;
	char buf[IWINFO_BUFSIZE];
